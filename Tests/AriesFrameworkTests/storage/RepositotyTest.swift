...
class RepositotyTest: XCTestCase {
    ...
    func testSave() async throws {
        ...
        do {
            try await repository.save(record)
            XCTFail("Duplicate error expected")
        } catch {
            let err = error as? ErrorCode
            if case .Duplicate = err {
                // expected
            } else {
                XCTFail("Duplicate error expected")
            }
    ...
    func testSaveAndGet() async throws {
        ...
        do {
            ...
            XCTFail("NotFound error expected")
        } catch {
            let err = error as? AriesFrameworkError
            if case .recordNotFoundError = err {
                // expected
            } else {
                XCTFail("NotFound error expected")
            }
    ...
    func testGetAll() async throws {
        ...
        let records = await repository.getAll()
    ...
    func testFindByQuery() async throws {
        ...
        let records = await repository.findByQuery("{\"myTag\": \"foobar\"}")
        XCTAssertEqual(records.count, 1)
        XCTAssertEqual(records[0].id, expectedRecord.id)

        let emptyRecords = await repository.findByQuery("{\"myTag\": \"notfound\"}")

...
public class Repository<T: BaseRecord & Codable> {
    ...
    public func getById(_ id: String) async throws -> T {
        guard let record = try await wallet.fetchRecord(
            category: T.type,
            id: id
        ) else {
    ...
    public func findByQuery(_ query: String) async -> [T] {
        do {
            let records = try await wallet.queryRecords(
                category: T.type,
                query: query
            )

...
public struct JweRecipient: Codable {
    /// Base64URL-encoded value of the encrypted CEK
    let encryptedKey: String
    let header: [String: String]?

    enum CodingKeys: String, CodingKey {
        case encryptedKey = "encrypted_key", header
    }
}

public struct JweEnvelope: Codable {
    let protected: String
    let unprotected: String?
    let recipients: [JweRecipient]?
    let ciphertext: String
    let iv: String
    let tag: String
    let aad: String?
    ...
    enum CodingKeys: String, CodingKey {
        case protected, unprotected, recipients, ciphertext, iv, tag, aad, header
        case encryptedKey = "encrypted_key"
    }
}

public struct ProtectedHeader: Codable {
    let enc: String
    let typ: String
    let alg: String
    let recipients: [JweRecipient]

...
public class CredentialService {
    ...
    */
    public func createProposal(options: CreateProposalOptions) async throws -> (ProposeCredentialMessage, CredentialExchangeRecord) {
        ...
        let credentialRecord = CredentialExchangeRecordBuilder()
            .setConnectionId(options.connection.id)
            .setThreadId(RecordUtils.generateId())
            .setProtocolVersion(CredentialsConstants.PROTOCOL_VERSION_V1)
    ...
    */
    ...
    func getHolderDid(credentialRecord: CredentialExchangeRecord) async throws -> String {
        let connection = try await agent.connectionRepository.getById(credentialRecord.connectionId ?? "connectionless")

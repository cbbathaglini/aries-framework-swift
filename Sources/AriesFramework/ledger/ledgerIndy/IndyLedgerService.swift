...
public class IndyLedgerService: LedgerService {
    ...
    public func getSchema(schemaId: String) async throws -> (String, Int) {
        logDebug("Get Schema with id: \(schemaId)")
    ...
    public func getCredentialDefinition(id: String) async throws -> String {
        logDebug("Get CredentialDefinition with id: \(id)")
    ...
    public func registerRevocationRegistryDefinition(did: DidInfo, revRegDefTemplate: RevocationRegistryDefinitionTemplate) async throws -> String {
        logDebug("registering revocation registry definition")
    ...
    public func getRevocationRegistryDefinition(id: String) async throws -> String {
        logDebug("Get RevocationRegistryDefinition with id: \(id)")
    ...
    public func getRevocationRegistryDelta(id: String, to: Int = Int(Date().timeIntervalSince1970), from: Int = 0) async throws -> (String, Int) {
        logDebug("Get RevocationRegistryDelta with id: \(id)")
    ...
    public func getRevocationRegistry(id: String, timestamp: Int) async throws -> (String, Int) {
        logDebug("Get RevocationRegistry with id: \(id), timestamp: \(timestamp)")
    ...
    public func revokeCredential(did: DidInfo, credDefId: String, revocationIndex: Int) async throws {
        logDebug("Revoking credential with index: \(revocationIndex)")
    ...
    public func getCredentialDefinitionVdr(credentialId: String) async throws -> indy_besu_vdr_uniffi.CredentialDefinition {
        throw CredoError("not implemented for indy")
    }
    ...
    public func getTailsPath() async -> String {
        ...
        let tailsFolderURL = documentsURL.appendingPathComponent("tails")
        
        if !fileManager.fileExists(atPath: tailsFolderURL.path) {
            try? fileManager.createDirectory(at: tailsFolderURL, withIntermediateDirectories: true)
        }
        
        return tailsFolderURL.path
    }

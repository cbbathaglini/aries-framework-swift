...
public class ProofService {
    ...
    */
    ...
    ) async throws -> (message: RequestPresentationMessageV2, record: ProofExchangeRecord) {
        try connectionRecord?.assertReady()

        let proofRequestJson = try JSONEncoder().encode(proofRequest)
        let attachment = Attachment.fromData(proofRequestJson, id: RequestPresentationMessageV2.INDY_PROOF_REQUEST_ATTACHMENT_ID)
        let message = RequestPresentationMessageV2(comment: comment, requestPresentationAttachments: [attachment])
    ...
    */
    ...
    /**
     Create a ``PresentationMessageV2`` as response to a received presentation request.
    ...
    */
    public func createPresentation(
        ...
        let proofRequestMessage = try JSONDecoder().decode(RequestPresentationMessageV2.self, from: proofRequestMessageJson.data(using: .utf8)!)
        ...
        let presentationMessage = PresentationMessage(
            comment: comment,
    ...
    /**
     ...
     - Returns: the presentation message and an associated proof record.
    */
    public func createPresentationV1(
        proofRecord: ProofExchangeRecord,
        requestedCredentials: RequestedCredentials,
        comment: String? = nil) async throws -> (message: PresentationMessage, record: ProofExchangeRecord) {
    ...
    */
    public func verifyProof(proofRequest: String, proof: String) async throws -> Bool {
        ...
        async let schemas = await getSchemas(schemaIds: Set(partialProof.identifiers.map { $0.schemaId }))
        async let credentialDefinitions = await getCredentialDefinitions(credentialDefinitionIds: Set(partialProof.identifiers.map { $0.credentialDefinitionId }))
        async let revocationRegistryDefinitions = await getRevocationRegistryDefinitions(revocationRegistryIds: Set(partialProof.identifiers.compactMap { $0.revocationRegistryId }))
    ...
    public func createProof(proofRequest: String, requestedCredentials: RequestedCredentials) async throws -> Data {
